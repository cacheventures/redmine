$(document).ready(function () {
  ...
  jQuery(document.body).on('click', '.link_to_modal', function () {
    ...
    if ( (cur_window.text() != '' || cur_window.hasClass('permanent_modal_window') || this_link.hasClass('static_content_only')) && !this_link.hasClass('refreshable') ){
      ...
      var hide_obj = this_link;
      if (this_link.hasClass('parent_loader')) {
        hide_obj = this_link.parents('.parent_loader:first');
      }
      var link = hide_obj.offset();
      var h = hide_obj.outerHeight();
      var w = hide_obj.outerWidth();
      hide_obj.addClass("invisible_link");
      $("#mw_content_load").css("left", link.left).css("top", link.top).css("width", w).css("height", h).show();

      if (this_link[0].tagName == 'A') {
        $.ajax({method: 'GET', url: this_link.attr('href')}).always(function (html) { cur_window.html(html.responseText || html); show_modal(id); });
      } else {
        var $frm = this_link.parents('form:first');
        $.ajax({ method: 'GET', url: $frm.attr('action'), data: $frm.serialize() }).always(function (html) { cur_window.html(html.responseText || html); show_modal(id); });
      }
...
function show_modal(id) {
  $("#"+id).removeClass("invisible_link");
  $("#"+id).parents('.parent_loader').removeClass("invisible_link");
  $("#mw_content_load").hide();
